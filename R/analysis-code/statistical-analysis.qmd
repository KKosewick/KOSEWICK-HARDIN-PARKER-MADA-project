...
Looking at our newly grouped occupations, we can see that our intercept has a high t-statistic and significance; this indicates that our reference category of Business/Finance jobs have a sleep quality score of about 7, which is pretty good.
...
for(i in 1:length(models)) {
  ...
  # Round the estimate and std.error to 2 decimal places
  model_results$estimate <- round(model_results$estimate, 2)
  model_results$std.error <- round(model_results$std.error, 2)
  model_results$statistic <- round(model_results$statistic, 3)  # Round to 3 decimal places
...
# Create a named vector of new term names for categorical variables
new_term_names <- c(
  "PhysicalActivity" = "PhysicalActivity.CategoryHigh(Reference: Low)",
  "Occupation_GroupEducation" = "Occupation_Education(Reference: BusinessFinance)",
  "Occupation_GroupEngineering" = "Occupation_Engineering(Reference: BusinessFinance)",
  "Occupation_GroupHealthcare" = "Occupation_Healthcare(Reference: BusinessFinance)",
  "Occupation_GroupLawyer" = "Occupation_Lawyer(Reference: BusinessFinance)",
  "Occupation_GroupScience" = "Occupation_Science(Reference: BusinessFinance)",
  "cat_bpStage 1 Hypertension" = "Stage_1_Hypertension(Reference: Elevated)",
  "cat_bpStage 2 Hypertension" = "Stage_2_Hypertension(Reference: Elevated)",
  "cat_bpNormal" = "Normal(Reference: Elevated)",
  "BMI.CategoryObese" = "BMI.CategoryObese(Reference: Normal)",
  "BMI.CategoryOverweight" = "BMI.CategoryOverweight(Reference: Normal)",
  "GenderMale" = "GenderMale(Reference: Female)"
  # Add more terms as needed
)

# Update the term names in the results
all_results$term <- ifelse(all_results$term %in% names(new_term_names), 
                           new_term_names[all_results$term], 
                           all_results$term)
...
flextable::save_as_image(ft, path = here("results", "tables", "table.png"))
...
It seems that stress level and quality of sleep are at an absolute value of 0.9 or higher on the correlation scale. Sleep Duration and stress level also seem closely tied. Stress level and quality of sleep are both subjective scales given by an individual; this is interesting to note.
...
We'll now train a Random Forest model using CV with no train/test split (as the data has less than 400 observations and many of the values are rare/unique for some columns). We'll follow the tidymodels framework.
...
This seems like a very solid fit; not too close to where overfitting is obvious, but not too far to where the predictions aren't useful either. The RMSE is extremely low (.059) and the R squared is extremely high (0.997). This is a good indicator that our selected model is powerful.
...
null_fit <- fit(null_mod, Quality.of.Sleep ~ ., data = sleepdatafinal)
...
We'll now train a LASSO model using CV with no train/test split (as the data has less than 400 observations and many of the values are rare/unique for some columns). We'll follow the tidymodels framework.
...
# Apply the prepared recipe to transform your data
processed_data <- bake(prepared_recipe, new_data = sleepdatafinal)
...
resamples2 <- vfold_cv(sleepdatafinal, v = 5, repeats = 5)

# Tune the LASSO model using cross-validation
lasso_tune <- tune_grid(
  lasso_wf,
  resamples = resamples2,  
  grid = lasso_grid,
...
final_wf2 <- finalize_workflow(lasso_wf, best_param)

# Fit the final LASSO model using the entire dataset
final_fit2 <- fit(final_wf2, data = sleepdatafinal)

# Extract the fitted LASSO model
fitted_model2 <- extract_fit_parsnip(final_fit2)

# Create the variable importance plot
vip(fitted_model2$fit)

```

*Kevin: Please check this*
...
predictions2 <- predict(final_fit2, new_data = sleepdatafinal)

# Add the predictions to dataframe

sleepdatafinal$Predicted <- predictions2$.pred
...
residuals2 <- sleepdatafinal$Quality.of.Sleep - predictions2$.pred

# Calculate RMSE

lasso_rmse <- sqrt(mean(residuals2^2))

# Calculate R squared

sst2 <- sum((sleepdatafinal$Quality.of.Sleep - mean(sleepdatafinal$Quality.of.Sleep))^2)

ssr2 <- sum(residuals2^2)

lasso_r2 <- 1 - (ssr2 / sst2)
...
The Observed vs. Predicted plot looks pretty good! Our RMSE value was once again quite low, at 0.0557 and our R-squared value was high at 0.997. Both of these metrics, combined with the Observed vs. Predicted plot indicate that the LASSO model is performing well on new data.
